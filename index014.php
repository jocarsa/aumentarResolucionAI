...
error_reporting(E_ERROR | E_PARSE);
    $resultados = "";
    $origen = imagecreatefromjpeg('test3.jpg');
    ...
    for($x = 0;$x<256;$x+=16){
        for($y = 0;$y<256;$y+=16){
            ...
            while (($linea = fgets($handle)) !== false) {
                ...
                if($contador < $mejorcandidato ){
                    $mejorcandidato = $contador;
                    $seleccionado = explode("|",$linea)[0];
                }
                //echo $contador."<br>";
                if($contenido == $cadena){
                    //echo "exito";
            ...
            //echo "origen: prueba/destino-".$x."-".$y.".jpg - Objetivo: ".$seleccionado.":".$mejorcandidato."<br>";
            $resultados .= $x.",".$y.",".$seleccionado.";";
        }
    }

    //echo $resultados;
    
    $explosion = explode(";",$resultados);

    for($i = 0;$i<count($explosion);$i++){
        $componente = explode(",",$explosion[$i]);
        echo "<img src='bloquesalta/".$componente[2]."' style='position:absolute;left:".$componente[0]."px;top:".$componente[1]."px;width:16px;height:16px;'>";
    }
